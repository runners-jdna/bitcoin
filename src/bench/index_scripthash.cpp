...
public:
    ...
    void PinHotSet(ScriptHashIndex& index) const
    {
        for (const auto& scripthash : m_hot_scripthashes) {
            index.CacheScriptHash(scripthash);
        }
    }

    void UnpinHotSet(ScriptHashIndex& index) const
    {
        for (const auto& scripthash : m_hot_scripthashes) {
            index.UncacheScriptHash(scripthash);
        }
    }
...
static void ScriptHashIndexHistoryMiniCold(benchmark::Bench& bench)
{
    ...
    size_t cursor{0};
    // Re-open the synced DB each iteration to keep the query path cold and make
    // cache-fill regressions visible, not just the warmed subscription path.
